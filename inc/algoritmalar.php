...
function PRICE($symbol){
 $apiLink = "https://www.binance.com/api/v3/ticker/price?symbol=".$symbol;
...
function SMA_RSI($symbol,$period,$time){
...
function EMA_RSI($symbol,$period,$time){
  $apiLink="https://www.binance.com/api/v3/klines?symbol=".$symbol."&interval=".$time;
  $fgc = file_get_contents($apiLink);
  $json=json_decode($fgc,true);
  $veri=array_reverse($json);
...
function WILLAMS_R($symbol,$period,$time){
  $apiLink="https://www.binance.com/api/v3/klines?symbol=".$symbol."&interval=".$time;
  $fgc = file_get_contents($apiLink);
  $json=json_decode($fgc,true);
  $veri=array_reverse($json);
