    ...
    class db
        {
            ...
            public function ekle($tabloAdi, $veriler) {
                //kullanım metodu
		        ...
		        $deger  .= "'";
                try {
                    $sql = "INSERT INTO $tabloAdi ($sutun) VALUES ($deger)";
                    $sonuc = $this->baglan->prepare($sql);
                    $sonuc->execute($veriler);
            ...
            public function ekleNormal($tabloAdi, $sutunlar, $degerler) {
                //kullanım metodu
                // ekle("TABLO", "sutun,sutun", array("sutundeğeri", "sutundeğeri"));
            ...
            public function guncelle($tabloAdi,$veriler,$kosul) {
                //kullanım metodu
                ...
                foreach ($veriler as $key => $val)
                {
                    ...
                    array_push($degerler,$val);
                }
                $sutunlar=explode(",", $sutunlar);
                $sutundeger="";
                foreach ($sutunlar as $sutun) {
                 $sutundeger .= ($sutundeger == "") ? "" : ", "; $sutundeger .= $sutun . "=?";
                }
                try {
            ...
            public function guncelleNormal($tip, $tabloAdi, $sutunlar, $kosul, $degerler) {
                //kullanım metodu
                //guncelle(TÜR, "TABLO", "SÜTUNLAR", "KOŞUL", array(değişecekdeğer,idnumarası));
                //guncelle(0, "haberler", "baslik,haber", "WHERE id=?", array("Değişen Başlik", "Değişen Iceriği", 10));
                //guncelle(1, "haberler", "okuma", "WHERE id=?", array(1, 10));
            ...
            public function sayfala($tip, $tabloAdi, $sutunlar, $kosul, $degerler, $toplamkayit, $sayfa, $link, $x) {
                ...
                $sayfala .="<li class='page-item'><a class='page-link' href=\"".$link."1\">&laquo; İlk</a></li>";
                $sayfala .="<li class='page-item'><a class='page-link' href=\"".$link.$onceki."\">Önceki</a></li>"; }
                    if($sayfa==1){ $sayfala .="<li class='page-item active'><a class='page-link'>1</a></li>";
                        }elseif($sayfa-$x < 2){ 
                            $sayfala .="<li class='page-item'><a class='page-link' href=\"".$link."1\">1</a></li>"; 
                        }
                        if($sayfa-$x > 2){ $i = $sayfa-$x; } else { $i = 2; }
                        if($sayfa-$x-10 > 0){ $sayfala .="<li class='page-item active'><a class='page-link' href=\"".$link.($sayfa-$x-10)."\">".($sayfa-$x-10)."</a></li>"; }
                            for($i; $i<=$sayfa+$x; $i++) { 
                                if($i==$sayfa){ $sayfala .="<li class='page-item active'><a class='page-link'>$i</a></li>"; } else { $sayfala .="<li class='page-item'><a class='page-link' href=\"".$link.$i."\">$i</a></li>"; }
                                if($i==$toplamsayfa) break; 
                            }
                        if($sayfa+$x+10 < $toplamsayfa){ $sayfala .="<li class='page-item active'><a class='page-link' href=\"".$link.($sayfa+$x+10)."\">".($sayfa+$x+10)."</a></li>"; }
                        if($sayfa < $toplamsayfa){
                            $sonraki = $sayfa+1; $sayfala .="<li class='page-item'><a class='page-link' href=\"".$link.$sonraki."\">Sonraki</a></li>";
                            $sayfala .="<li class='page-item'><a class='page-link' href=\"".$link.$toplamsayfa."\">Son &raquo;</a></li>"; 
                        }
                    }
        ...
        function admin_oturum_kontrol_panel()
        {
            if (isset($_SESSION['username'])) {
                ...
                $giriskontrol = $baglan->cek("OBJ","kullanicilar","e_posta,parola","where e_posta ='$kullanici' and parola ='$sifre'",array());    
        ...
        function admin_oturum_kontrol_giris()
        {
            if (isset($_SESSION['username'])) {
                    $baglan = new db;
                    $kullanici = $_SESSION['username'];
